...
export async function addTodo(todo: Omit<Todo, 'id' | 'fait' | 'progression' | 'notes_personnelles' | 'recurrence'> & { recurrence?: string }): Promise<void> {
  const db = await getDatabase();
  await db.runAsync(
    `INSERT INTO todo (titre, description, date, heure_pensee, priorite, categorie, cours_id, recurrence)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [todo.titre, todo.description, todo.date, todo.heure_pensee, todo.priorite, todo.categorie || '', todo.cours_id, todo.recurrence || 'none']
  );
  cacheTodos = {};
}
...
export async function updateTodo(id: number, todo: Partial<Todo>): Promise<void> {
  ...
  if (todo.categorie !== undefined) { fields.push('categorie=?'); values.push(todo.categorie); }
  if (todo.recurrence !== undefined) { fields.push('recurrence=?'); values.push(todo.recurrence); }
...
export async function completeAndRepeat(todo: Todo): Promise<void> {
  ...
  if (todo.recurrence && todo.recurrence !== 'none') {
    ...
    await db.runAsync(
      `INSERT INTO todo (titre, description, date, heure_pensee, priorite, categorie, cours_id, recurrence)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [todo.titre, todo.description, newDate, todo.heure_pensee, todo.priorite, todo.categorie || '', todo.cours_id, todo.recurrence]
    );
  }
